...
, path
, selfPackages
, tmpDir ? "/tmp/proxyon-nvfetcher-packages-update"
...
let
  ...
  drv = writeShellScriptBin "update" ''
    ...
    pushd nix/pkgs;
    set -x
    "${main}/bin/main" "''${NVCHECKER_EXTRA_OPTIONS[@]}";
    set +x
    popd
    nix fmt

    # get changelog
    ${commit}/bin/commit "nix/pkgs/_sources/generated.nix" > "${nvfetcherChangelogFile}"
